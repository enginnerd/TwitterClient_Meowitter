...
//        myHandler.postDelayed(mTimelineRunnable, 100); //wait 1.5 seconds till running so we get the updated data from twitter
    ...
    private void populateTimeline_real(boolean dontClear){
        ...
        Log.i("DEBUG_USERTIMELINE", "Screen name: " + screenName);
        client.getUserTimeline(screenName, new JsonHttpResponseHandler() {
            @Override
            public void onSuccess(int statusCode, Header[] headers, JSONArray json) {
                Log.d("DEBUG-USERTIMELINE", json.toString());
                    /*ArrayList<Tweet> tweets = */
                Tweet.fromJSONArray(json, Tweet.PROFILE, screenName); //add to DB
                last_id = getTweets(f_dontClear, last_id, Tweet.PROFILE, screenName);
...
//                        getTweets(f_dontClear);
                ...
                getTweets(f_dontClear, last_id, Tweet.MENTIONS, screenName);
